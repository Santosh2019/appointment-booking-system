...
public class AuthController {
    ...
    @Autowired
    private PatientFeignClient patientFeignClient;  // ← Inject your Feign client for patient-service
    ...
    public String performLogin(@RequestParam("email") String email,
        ...
        log.info("=== PERFORM LOGIN METHOD STARTED === Email: {}", email);
        ...
        if (!isAuthServiceAvailable()) {
            ...
            return fallbackLogin(email, dob, session, response);
        }
        ...
        try {
            ...
            if (authResponse.getStatusCode().is2xxSuccessful()) {
                ...
                // ── NEW: Fetch patient details and store in session ──
                try {
                    log.info("Attempting to fetch patient via Feign for email: {}", email);
                    ...
                    log.info("Login successful | email = {}, patientId = {}, name = {}, token issued",
                            email, patient.getPatientId(), patient.getFullName(), token.length());
                } catch (Exception e) {
                    log.warn("Failed to fetch patient details after login for email = {}", email, e);
                }
    ...
    private String fallbackLogin(String email, String dobStr, HttpSession session, HttpServletResponse response) {
        log.info("Fallback login triggered | email = {}, dob = {}", email, dobStr);
        log.info("=== FALLBACK LOGIN METHOD STARTED === Email: {}", email);
        try {
            ...
            if (user.getDob().toString().equals(dobStr)) {
                ...
                // ── NEW: Store in session (using User data; adjust fields if needed) ──
                // If User has patientId field, use it; otherwise fallback to another lookup
                // Here assuming User has or can map to patientId – change as per your User entity
                session.setAttribute("loggedInPatientId", user.getId());  // ← Adjust to actual patientId field
                session.setAttribute("loggedInPatientName", user.getId() != null ? user.getEmail() : email);
                log.info("Fallback login SUCCESS | email = {}, patientId = {}, name = {}, token issued",
                        email, session.getAttribute("loggedInPatientId"), session.getAttribute("loggedInPatientName"), token.length());
    ...
    // ... rest of the class unchanged (logout, setJwtCookie, generateJwtToken, isAuthServiceAvailable) ...
